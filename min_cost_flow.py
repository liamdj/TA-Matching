...
class MatchingGraph:
    ...
    def __init__(self, match_weights, student_weights, course_weights, course_slots, fixed_matches):
        ...
        for _, row in fixed_matches.iterrows():
            ...
            if si == -1:
                ...
                self.flow.SetNodeSupply(self.num_students + ci, self.flow.Supply(self.num_students + ci) + 1)
            elif ci == -1:
                missing += 1
            else:
                ...
                self.flow.AddArcWithCapacityAndUnitCost(si, self.num_students + ci, 1, match_cost + assign_cost)
    ...
    def get_matching(self, fixed_matches):
        ...
        for arc in range(self.flow.NumArcs()):
            si, ci = self.flow.Tail(arc), self.flow.Head(arc) - self.num_students
            ...
            # arcs from source to student
                ...
                rows = fixed_matches.loc[fixed_matches['Student index'] == self.flow.Head(arc)]
    ...
    def write_matching(self, filename, weights, student_data, student_scores, course_data, course_scores, fixed_matches):
        ...
        matches.sort(key=lambda tup: 100 if tup[1] == -1 else (-100 if (fixed_matches == tup).all(1).any() else -weights[tup[0], tup[1]]))
        ...
        with open(filename, 'w', newline='') as file:
            ...
            for i, (si, ci) in enumerate(matches):
                ...
                notes = []
                if i < len(fixed_matches.index):
                    notes.append("fixed")
                if student_data.loc[student, "Weight"] < 0:
                    notes.append("negative student weight")
