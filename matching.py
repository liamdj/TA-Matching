...
student_data_cols = ['Name', 'Weight', 'Previous', 'Advisors', 'Notes']
...
def get_student_scores(data: pd.DataFrame, cols: pd.Index) -> pd.DataFrame:
    ...
    for index, row in data.iterrows():
        ...
        rows.append(pd.Series({c: score for (c, _), score in
                               zip(student_rankings(row, cols), scores)},
                              name=index))
...
def get_course_scores(data: pd.DataFrame, cols: pd.Index) -> pd.DataFrame:
    ...
    for index, row in data.iterrows():
        ...
        scores = [float(v) for v in values]
        rows.append(pd.Series({c: score for (c, _), score in
                               zip(course_rankings(row, cols), scores)},
                              name=index))
    return pd.concat(rows, axis='columns').T
...
def match_weights(student_data: pd.DataFrame, student_scores: pd.DataFrame,
    ...
    weights = np.full((len(student_scores.index), len(course_scores.index)),
                      np.nan)
...
def read_student_data(filename: str) -> pd.DataFrame:
    ...
    df['Weight'] = pd.to_numeric(df['Weight'], errors='coerce',
        downcast='float').fillna(params.DEFAULT_ASSIGN)
    return df
...
def read_course_data(filename: str) -> pd.DataFrame:
    ...
    df['First weight'] = pd.to_numeric(df['Weight'], errors='coerce',
        downcast='float').fillna(params.DEFAULT_FIRST_FILL)
...
def check_input(student_data: pd.Series, course_data: pd.Series):
    for index in student_data[student_data.index.duplicated()].index:
        sys.exit('Duplicate rows for netid {}. Exiting without solving'.format(
            index))
    for index in course_data[course_data.index.duplicated()].index:
        sys.exit('Duplicate rows for course {}. Exiting without solving'.format(
            index))
...
def matching_differences(extra_course: str, old: List[Tuple[int, int]],
    ...
    cmo = {course: ';'.join(sorted(students)) for course, students in
           course_matches_old.items()}
    cmn = {course: ';'.join(sorted(students)) for course, students in
           course_matches_new.items()}
    df_c = pd.concat([pd.Series(cmo, dtype=str), pd.Series(cmn, dtype=str)],
                     axis=1).fillna('none')
...
def test_additional_TA(path, student_data, course_data, student_scores,
    ...
    for ci, course in enumerate(course_data.index):
        ...
        fixed_edit = pd.concat([fixed_matches.T, pd.Series(['', course, -1, ci],
            index=['Student', 'Course', 'Student index', 'Course index'])],
                               axis=1).T
        graph_edit = min_cost_flow.MatchingGraph(weights,
            student_data['Weight'],
            course_data[['Slots', 'Base weight', 'First weight']], fixed_edit)
        if graph_edit.solve():
            new_matches = graph_edit.get_matching(fixed_edit)
            student_changes, course_changes = matching_differences(course,
                initial_matches, new_matches, student_data, course_data,
    ...
    with open(path, 'w', newline='') as file:
        ...
        writer.writerow(['Course', 'Weight change', 'Student differences',
                         'Course differences'])
...
def test_removing_TA(path, student_data, course_data, student_scores,
    ...
    for si, student in enumerate(student_data.index):
        if si not in fixed_matches['Student index'].values:
            ...
            fixed_edit = pd.concat([fixed_matches.T,
                                    pd.Series([student, '', si, -1],
            ...
            if graph_edit.solve():
                new_matches = graph_edit.get_matching(fixed_edit)
                student_changes, course_changes = matching_differences(None,
                    initial_matches, new_matches, student_data, course_data,
                    student_scores, course_scores)
                data[student] = (
                                            graph.flow.OptimalCost() - graph_edit.flow.OptimalCost()) / 10 ** min_cost_flow.DIGITS, single_line(
    ...
    with open(path, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Student', 'Weight change', 'Student differences',
                         'Course differences'])
...
def find_alternate_matching(path, student_data, course_data, weights,
    ...
    while True:
        ...
        if graph_edit.solve():
            ...
            student_changes, course_changes = matching_differences(None,
                last_matches, new_matches, student_data, course_data,
                student_scores, course_scores)
...
def run_matching(path="", student_data="inputs/student_data.csv",
    ...
    weights = match_weights(student_data, student_scores, course_data,
                            course_scores)

    if os.path.isfile(path + adjusted):
        adjusted_matches = pd.read_csv(path + adjusted,
            dtype={'Netid': str, 'Course': str, 'Weight': float})
    ...
    graph = min_cost_flow.MatchingGraph(weights, student_data['Weight'],
    ...
    test_additional_TA(path + output + 'additional_TA.csv', student_data,
                       course_data, student_scores, course_scores, weights,
                       fixed_matches, graph)
    test_removing_TA(path + output + 'remove_TA.csv', student_data, course_data,
                     student_scores, course_scores, weights, fixed_matches,
                     graph)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', metavar='FOLDER', default='.',
    ...
    parser.add_argument('--course_data', metavar='COURSE DATA',
                        default='inputs/course_data.csv',
                        help='csv file with course data rows')
    parser.add_argument('--fixed', metavar='FIXED INPUT',
                        default='inputs/fixed.csv',
                        help='csv file with fixed student-course matchings')
    parser.add_argument('--adjusted', metavar='ADJUSTED INPUT',
                        default='inputs/adjusted.csv',
    ...
    parser.add_argument('--alternates', metavar='ALTERNATE MATCHES', type=int,
                        default=3,
