...
if __name__ == "__main__":
    ...
    with open("student_info.csv", "w", newline='') as info_file,  open("student_pref.csv", 'w', newline='') as pref_file:
        ...
        for i in range(STUDENTS):
            ...
            previous = ';'.join(np.random.choice(ordering[0:nums[1]], size=np.random.choice(3, p=[0.5, 0.4, 0.1]), replace=False))
            advisors = np.random.choice(ordering[0:nums[0]]) if np.random.randint(2) == 0 else ''
    ...
    students = ["Student #{}".format(i) for i in range(STUDENTS)]
    with open("prof_pref.csv", "w", newline='') as file:
        ...
        for course, slots in courses.itertuples():
            ...
            poor = ';'.join(ordering[nums[1]:nums[2]])
            unqualified = ';'.join(ordering[nums[2]:nums[3]])
