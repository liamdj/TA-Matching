...
if __name__ == "__main__":
    try:
        ...
        sys.exit("Required arguments are courses filename and student delta")
   
    courses = pd.read_csv(filename, index_col="Course", dtype = {"Course": str, "TA Slots": int}, usecols=["Course", "TA Slots"])
    courses["TA Slots"].fillna(1, inplace=True)
    total_slots = courses["TA Slots"].sum()

    course_slots = np.array([course for course, slots in courses.itertuples() for _ in range(slots)]).astype(str)

    with open("student_info.csv", "w", newline='') as info_file,  open("student_prefs.csv", 'w', newline='') as pref_file:
        ...
        for i in range(total_slots + delta):
    ...
    students = ["Student #{}".format(i) for i in range(total_slots + delta)]
    with open("prof_prefs.csv", "w", newline='') as file:
